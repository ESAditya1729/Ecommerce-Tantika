...
const OrderModal = ({ isOpen, onClose, product }) => {
  ...
  // USER STATES - Get from localStorage
  const [userId, setUserId] = useState(null);
  const [userEmail, setUserEmail] = useState('');

  const API_URL = process.env.REACT_APP_API_URL || 'https://ecommerce-tantika.onrender.com/api';
  ...
  // FETCH ADDRESSES WHEN USER ID IS AVAILABLE
  useEffect(() => {
    if (isOpen && userId) {
      fetchSavedAddresses();
    }
  ...
  const selectAddress = (address) => {
    ...
    // Also set email if not already set
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const shippingAddress = `${formData.addressLine1}${formData.addressLine2 ? ', ' + formData.addressLine2 : ''}, ${formData.city}, ${formData.state} - ${formData.pincode}, ${formData.country}`;
      ...
      if (data.success) {
        ...
        throw new Error(data.message || data.error || 'Failed to submit interest');
      }
  ...
  // RENDER ADDRESS DROPDOWN SECTION
            ...
            <div className="flex items-center">
              ...
              <span className="text-sm">
                {loadingAddresses 
                  ? 'Loading addresses...' 
                  : selectedAddressId 
                    ? `Selected: ${savedAddresses.find(a => a._id === selectedAddressId)?.name || 'Address'}`
              ...
              <div className="p-2">
                {loadingAddresses ? (
                  ...
                  savedAddresses.map((address) => {
                    const Icon = address.type === 'work' ? Building : address.type === 'other' ? Briefcase : Home;
                    const color = address.type === 'work' ? 'text-green-600' : address.type === 'other' ? 'text-purple-600' : 'text-blue-600';
              ...
              <div className="mt-4 p-4 bg-white/50 border border-green-100 rounded-lg">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-gray-900">{orderDetails.productName || product.name}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Amount:</span>
                    <span className="font-medium text-green-600">₹{orderDetails.totalAmount || product.price}</span>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-gray-600 text-sm mb-2">
                    By {product.artisan || 'Artisan'} • {product.location || 'India'}
                  </p>
              ...
              <form onSubmit={handleSubmit} className="p-6">
                ...
                <div className="mb-6">
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    disabled={isSubmitting}
                    className="w-full px-4 py-3 border border-gray-300 rounded-lg bg-gray-50 outline-none transition-all disabled:opacity-50"
                  />
                </div>
